 ...
 * ============================================================================
 *
 * Yaad mat karo 15 cases. Sirf 2 rules + 4 buckets.
 *
 * RULE 1: Normal fn → this CALL-TIME pe
 * RULE 2: Arrow fn  → this WRITE-TIME pe (enclosing scope)
 *
 * DECISION TREE:
 *   1. Arrow hai?
 *      HAAN → enclosing function/scope ka this. STOP.
 *      NAHI → step 2
 *   2. Kaise call hua?
...
const user1 = {
    ...
    inner: {
        name: "Inner",
        greet() {
            // Dot ke TURANT left = inner (outer user1 nahi)
...
// Explicit binding bhi isi bucket mein aata hai (call/apply)
function showName() {
...
// ============================================================================
// Method copy, setTimeout(fn), forEach(normalFn) — sab same problem
// ============================================================================
...
const user2 = {
    ...
    printMembers() {
        this.members = ["A", "B"];
        // forEach normal callback ko standalone chalata hai (setTimeout jaisa)
        this.members.forEach(function (m) {
            // this yahan team/user2 NAHI — bare callback
...
// ============================================================================
// BUCKET 3: Arrow INSIDE a normal method  →  parent method ka this
// ============================================================================
...
const service3 = {
    ...
    start() {
        ...
        setTimeout(() => {
            // Arrow: apna this nahi → start ka this inherit
    ...
    printMembers() {
        ...
        this.members.forEach((m) => {
            // Arrow → printMembers ka this = service3
            console.log("B3 forEach arrow:", this.id, m);
        });
    },
    // Method se arrow RETURN → creation-time this LOCK
    getPrinter() {
        return () => {
...
printer(); // Expected: "DB_1"  (standalone chalega phir bhi — arrow lexical)
printer.call({ id: "HACK" }); // Expected: still "DB_1" (call arrow this nahi badalta)


// ============================================================================
// BUCKET 4: Arrow as object method / top-level arrow  →  module/enclosing scope
// ============================================================================
// Object {} scope NAHI banata — isliye arrow object ka this nahi leta
// ============================================================================
...
const user4 = {
    name: "Umakant",
    // ❌ Arrow as method — object ko this nahi milta
    say: () => {
    ...
    inner: {
        name: "Inner",
        say: () => {
            // Nested object bhi scope nahi — ab bhi module this
            console.log("B4 nested object arrow:", this && this.name);
        }
    }
};

user4.say();       // Expected: undefined (Node module — name nahi)
...
│  BUCKET 3  Arrow inside method   → parent method ka this    │
